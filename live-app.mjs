...
import {createImageryProtocol,CAMERA_MAX_ZOOM,IMAGERY_MAX_ZOOM} from './imagery-source.mjs';
import {createTerrainProtocol} from './terrain-source.mjs';
...
if(cameraParts?.length===5&&cameraParts.every(Number.isFinite)&&Math.abs(cameraParts[1])<=85&&cameraParts[2]>=0&&cameraParts[2]<=CAMERA_MAX_ZOOM&&cameraParts[4]>=0&&cameraParts[4]<=75){selected=[((cameraParts[0]+180)%360+360)%360-180,cameraParts[1]];initialCamera={center:selected,zoom:cameraParts[2],bearing:cameraParts[3],pitch:cameraParts[4]};view3d=cameraParts[4]>0;region='';}
const dem={type:'raster-dem',tiles:['terrain-dem://{z}/{x}/{y}?usage=mesh'],tileSize:512,encoding:'terrarium',maxzoom:18,attribution:'<a href="https://mapterhorn.com/attribution/" target="_blank" rel="noopener">© Mapterhorn · 표고 출처</a>'};
...
let map;
let imageryProtocol,terrainProtocol,qualityEnabled=true,imageryError=false;
const imageryTiles=new Map(),terrainTiles=new Map();
function rememberTile(cache,meta){const key=`${meta.requestedZoom}/${meta.x}/${meta.y}`;cache.delete(key);cache.set(key,meta);while(cache.size>192)cache.delete(cache.keys().next().value);renderMapQuality();}
function centerTile(cache){if(!map)return null;const center=map.getCenter(),lng=((center.lng+180)%360+360)%360-180,sin=Math.sin(Math.max(-85,Math.min(85,center.lat))*Math.PI/180);let best=null;for(const meta of cache.values()){if(meta.requestedZoom>Math.ceil(map.getZoom())+1)continue;const n=2**meta.requestedZoom,x=Math.floor((lng+180)/360*n),y=Math.floor((.5-Math.log((1+sin)/(1-sin))/(4*Math.PI))*n);if(meta.x===x&&meta.y===y&&(!best||meta.requestedZoom>best.requestedZoom))best=meta;}return best;}
function renderMapQuality(){
 if(!map)return;
 const zoom=map.getZoom(),imagery=centerTile(imageryTiles),terrain=centerTile(terrainTiles);
 $('#imagery-zoom-value').textContent=`${zoom.toFixed(1)} / ${CAMERA_MAX_ZOOM}`;
 $('#live-pitch').value=Math.round(map.getPitch());$('#live-pitch-value').textContent=`${Math.round(map.getPitch())}°`;
 const beyond=imagery&&zoom+1>imagery.nativeZoom+.1;
 $('#imagery-quality-status').textContent=imagery?`중앙 영상 L${imagery.nativeZoom} · ${beyond?'원본 한계 이후 확대 보간': '제공 원본 사용'}${imagery.enhanced?' · 선명도 보정':qualityEnabled?' · 근접 확대 시 자동 보정':' · 보정 끔'}`:'화면 중앙의 원본 영상 확인 중…';
 if(imageryError)$('#imagery-quality-status').textContent='일부 영상의 원본을 확인하지 못했습니다. 연결을 확인하고 지도를 다시 이동해주세요.';
 $('#imagery-quality-status').dataset.state=imageryError?'error':beyond?'interpolated':imagery?'native':'loading';
 if(terrain)$('#terrain-detail').textContent=`중앙 표고 L${terrain.nativeZoom} · ${terrain.nativeZoom>12?'지역 정밀 표고':'전 지구 기본 표고'}${terrain.overzoomed?' · 확대 보간':''}. 높이 1×; 건물 입체 모델은 별도 자료가 필요합니다.`;
 $('#live-zoom-in').disabled=zoom>=CAMERA_MAX_ZOOM-.01;$('#live-zoom-out').disabled=zoom<=.01;
}
function imageryURL(){return `land15-imagery://${qualityEnabled?'enhanced':'original'}/{z}/{x}/{y}`;}
function setMapSubtab(name){$$('[data-map-subtab]').forEach(button=>{const active=button.dataset.mapSubtab===name;button.setAttribute('aria-selected',String(active));button.tabIndex=active?0:-1;});$$('[data-map-subpane]').forEach(pane=>pane.hidden=pane.dataset.mapSubpane!==name);}
...
function setLocation(coordinate,name){selected=[Number((((coordinate[0]+180)%360+360)%360-180).toFixed(5)),Number(coordinate[1].toFixed(5))];$('#live-coordinate').textContent=coordinateLabel(selected);$('#live-location').textContent=name||'선택 지점';$('#coordinate-lat').value=selected[1];$('#coordinate-lng').value=selected[0];const googleCoordinate=encodeURIComponent(`${selected[1]},${selected[0]}`);$('#live-google-earth').href=`https://earth.google.com/web/search/${googleCoordinate}`;$('#live-street-view').href=`https://www.google.com/maps/@?api=1&map_action=pano&viewpoint=${googleCoordinate}`;for(const id of ['live-google-earth','live-street-view'])$('#'+id).title=`선택 좌표 ${selected[1]}, ${selected[0]} · Google에서 열기`;if(mapReady)map.getSource('point').setData({type:'Feature',geometry:{type:'Point',coordinates:selected},properties:{}});}
...
function set3d(enabled){view3d=enabled;$('#live-3d').setAttribute('aria-pressed',String(enabled));$('#live-2d').setAttribute('aria-pressed',String(!enabled));if(map?.getSource('dem')){map.setTerrain(enabled?{source:'dem',exaggeration:1}:null);map.easeTo({pitch:enabled?60:0,duration:reduced.matches?0:650});terrainStatus();}}
function evidenceHTML(){
 ...
 <h3>실제 3D 지형</h3><p>Mapterhorn의 Terrarium 공개 표고를 높이 배율 1로 표시합니다. 전 지구 기본 표고는 30m급이며 지역별 정밀 표고가 있는 곳은 제공 목록을 확인해 최대 L18까지 자동으로 선택합니다. 자료가 없는 확대 단계는 실제 부모 표고를 높이 값에서 보간하며 새로운 실측 지형을 생성하지 않습니다. 표고에 나무·건물 영향이 포함될 수 있습니다. 음영은 실제 표고의 경사와 방향에서 계산한 약한 다방향 조명이며 위성 촬영 당시의 그림자를 재현한 것이 아닙니다. Google Earth와 주변 Street View는 선택 좌표의 공식 외부 화면으로 연결합니다. 건물·나무의 실사 3D와 파노라마는 제공 지역에 한정됩니다. 이 사이트 안에 Google 실사 3D를 표시하려면 별도의 API 설정이 필요합니다.</p><ul><li>${sourceLink('https://mapterhorn.com/attribution/','표고 제공기관과 원자료')}</li><li>${sourceLink('https://www.arcgis.com/home/item.html?id=10df2279f9684e4a9f6a7f08febac2a9','Esri World Imagery')}</li><li>${sourceLink('https://developers.google.com/maps/documentation/tile/3d-tiles','Google 실사 3D 연결 조건')}</li></ul>
 <h3>확대와 선명도 보정</h3><p>카메라 확대는 최대 22단계입니다. Esri의 타일 가용성 정보를 확인해 실제 제공되는 영상만 표시하며, 더 높은 해상도가 없는 곳은 마지막 원본의 해당 영역을 확대합니다. 화질·실사 탭에서 중앙 영상의 원본 단계와 확대 보간 여부를 확인할 수 있습니다. 선명도 보정은 근접 확대에서만 영상의 기존 명암 경계를 제한적으로 조절합니다. 없어진 글자·건물·관측 정보를 복원하거나 실제 공간 해상도를 높이는 기능이 아닙니다. 보정을 끄면 원본 표시로 비교할 수 있습니다. 이 과정은 표고·NASA·황폐화 분류·수치 분석에 적용되지 않으며 수정한 영상은 서버나 영구 저장소에 저장하지 않습니다.</p>
...
function bindUI(){
 const mapTabs=$$('[data-map-subtab]');mapTabs.forEach((button,index)=>{button.addEventListener('click',()=>setMapSubtab(button.dataset.mapSubtab));button.addEventListener('keydown',event=>{if(!['ArrowLeft','ArrowRight','Home','End'].includes(event.key))return;event.preventDefault();const next=event.key==='Home'?0:event.key==='End'?mapTabs.length-1:(index+1)%mapTabs.length;setMapSubtab(mapTabs[next].dataset.mapSubtab);mapTabs[next].focus();});});
 $('#imagery-quality-toggle').addEventListener('change',()=>{qualityEnabled=$('#imagery-quality-toggle').checked;imageryTiles.clear();imageryError=false;map?.getSource('satellite')?.setTiles([imageryURL()]);renderMapQuality();});
 $('#live-pitch').addEventListener('input',()=>{if(!map?.getSource('dem'))return;if(!view3d&&Number($('#live-pitch').value)>0){view3d=true;map.setTerrain({source:'dem',exaggeration:1});$('#live-3d').setAttribute('aria-pressed','true');$('#live-2d').setAttribute('aria-pressed','false');}map.setPitch(Number($('#live-pitch').value));renderMapQuality();});
...
function initMap(){
 try{if(!window.maplibregl)throw Error('MapLibre unavailable');imageryProtocol=createImageryProtocol({onTile:meta=>rememberTile(imageryTiles,meta)});terrainProtocol=createTerrainProtocol({onTile:meta=>{if(meta.usage==='mesh')rememberTile(terrainTiles,meta);}});maplibregl.addProtocol('land15-imagery',imageryProtocol);maplibregl.addProtocol('terrain-dem',terrainProtocol);map=new maplibregl.Map({container:'map',...initialCamera,minZoom:0,maxZoom:CAMERA_MAX_ZOOM,maxPitch:75,attributionControl:{compact:true},renderWorldCopies:false,style:{version:8,projection:{type:'globe'},sources:{satellite:{type:'raster',tiles:[imageryURL()],tileSize:256,maxzoom:IMAGERY_MAX_ZOOM,attribution:'Imagery © Esri, Vantor, Earthstar Geographics, and the GIS User Community'},dem:{...dem},'dem-shading':{...dem,tiles:['terrain-dem://{z}/{x}/{y}?usage=shade']}},layers:[{id:'background',type:'background',paint:{'background-color':'#253f3a'}},{id:'satellite',type:'raster',source:'satellite',paint:{'raster-saturation':0,'raster-contrast':0}},{id:'terrain-shading',type:'hillshade',source:'dem-shading',paint:shadePaint}],terrain:view3d?{source:'dem',exaggeration:1}:undefined}});
 map.on('load',()=>{mapReady=true;renderMapQuality();map.addSource('point',{type:'geojson',data:{type:'Feature',geometry:{type:'Point',coordinates:selected},properties:{}}});map.addLayer({id:'point-halo',type:'circle',source:'point',paint:{'circle-radius':11,'circle-color':'#142a27','circle-stroke-color':'#e7f4e7','circle-stroke-width':1.5,'circle-opacity':.65}});map.addLayer({id:'point-dot',type:'circle',source:'point',paint:{'circle-radius':3,'circle-color':'#f3fff1'}});terrainStatus();updateViewportRaster();queryPoint(selected,{name:regions[region]?.name});});
 map.on('zoom',renderMapQuality);map.on('pitch',renderMapQuality);map.on('moveend',renderMapQuality);
 ...
 map.on('sourcedata',event=>{if(event.sourceId==='satellite'&&event.sourceDataType==='content'&&event.isSourceLoaded){imageryError=false;renderMapQuality();}});map.on('error',event=>{if(event.sourceId==='satellite'){imageryError=true;renderMapQuality();}});
...
window.addEventListener('pagehide',()=>{queryController?.abort();gridController?.abort();observationLoader.cancel();imageryProtocol?.dispose();terrainProtocol?.dispose();clearTimeout(queryTimer);},{once:true});
