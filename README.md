...
브라우저에서 http://127.0.0.1:4173 을 엽니다. 위성영상·표고에는 인터넷 연결이 필요합니다. 지도 렌더러 MapLibre GL JS 5.14.0과 SUIT 한글 가변 글꼴은 vendor/에 포함되어 있습니다.

## 주요 기능

- **연도 비교:** 상단 `연도 비교`에서 2001·2010·2020·2025년 8월 NASA MODIS 식생지도를 같은 위치에서 좌우로 비교합니다. 경계 슬라이더와 키보드로 조절하고, 고비·사헬·아랄 대표점의 NDVI 및 연강수 그래프를 봅니다. 모바일에서는 `지도 비교 / 지표 변화`로 전환합니다.
- **위험 요인 설명:** 분석 탭의 `위험 요인 보기` 또는 지도 셀을 선택하면 합성 NB 입력 하나를 교육용 기준값으로 바꾼 점수 차이 상위 3개를 보여줍니다. 실제 인과관계·기여율이 아니며 NB 초기 점수와 CA 보정 점수를 구분합니다.
- **발표 모드:** `문제 지역 → 위험 요인 → 복원 후보 → 전후 비교 → 결과 요약`을 카메라 이동과 함께 진행합니다. 시작 시 분석을 고정하고 종료 시 원래 지도·설정을 복원합니다. 8초 자동 재생, 일시정지, 단계 이동, PNG 결과 카드가 있습니다.
...
- [Taste Skill의 Redesign Skill](https://github.com/Leonxlnx/taste-skill/blob/main/skills/redesign-skill/SKILL.md)을 참고한 차콜·민트 색상과 SUIT 글꼴. 기존 3D 지형과 떠 있는 패널 배치를 유지하고, 21st.dev Motion Primitives Dock의 확대·스프링·툴팁을 보존했습니다. 네 개의 탭 이름은 항상 표시하며 작은 화면은 간격을 조정해 본문 스크롤 없이 조작합니다. 설계 원칙은 [DESIGN.md](DESIGN.md)에 기록했습니다.
...
- `history-view.mjs`, `data/history.json`: 실제 과거 식생지도 비교와 대표점 지표. 자료 범위·품질·재현법은 [data/history-sources.md](data/history-sources.md)에 있습니다.
- `explanation.mjs`: 합성 NB 입력을 하나씩 대체한 민감도 설명
- `presentation.mjs`, `presentation.css`: 발표 진행과 PNG 결과 카드
- `experiences.css`: 연도 비교 및 셀 설명 반응형 UI
...
node --test explanation.test.mjs history.test.mjs presentation.test.mjs
node build.mjs
```
...
2026-09-14 검증: 총 52개 자동 테스트 통과. 실제 NDVI 자료의 2001·2010·2020·2025년 8월 지도 비교와 대표점 자료의 품질 미충족·결측을 `자료 없음`으로 표시하는 동작을 확인했습니다. 390×667 모바일·844×390 가로 화면·1366×768 데스크톱에서 연도 비교, 위험 요인 설명, 5단계 발표 모드를 확인했습니다.

## 알려진 한계

- 연도 비교 지도는 8월 월 합성, NDVI 그래프는 250m 대표 픽셀의 8월 중순 16일 합성입니다. 대표점 자료는 지역 전체 평균이 아닙니다. 사헬 2020·2025년 NDVI는 구름 품질 기준 미충족으로 `자료 없음`을 표시합니다. 그래프를 위험 모델의 합성값에 덮어쓰지 않습니다.
- 연강수는 NASA POWER/MERRA-2 월평균 일강수량에 월 일수를 곱한 근사 합계입니다. 현장 우량계 관측이나 원격 탐사 픽셀과 동일한 해상도가 아닙니다.
- 발표 카드는 시작 시점의 합성 분석 결과입니다. 복원 가정으로 바뀌는 점수는 실제 복원 효과나 미래 예측을 뜻하지 않습니다.
